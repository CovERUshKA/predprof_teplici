...
const url = "http://45.147.177.204:80\/api/sensors_data?time_period=100";
// ссылка для получения состояний систем (полива, окна, увлажнения)
const url_states = "http://45.147.177.204:80\/api/state";
// ссылка для получения устновленных параметров T, H, Hb
const url_system = `http://45.147.177.204:80\/api/parameters`;
// ссылка для ручного ввода показаний с датчиков
const url_set_sensors_data = "http://45.147.177.204:80\/api/add_data";
...
async function switch_system(system = '', watering_id=0){
    let url_system = `http://45.147.177.204:80\/api/${system}`;
    let patch_body = {};
